...
| Additional folders under `Scripts/Runtime/`: `Buildings/`, `Units/`, `Combat/`, `Pathfinding/`, `Placement/`, `Selection/`, `Pooling/`, `Events/`, `UI/Production/`, `UI/Info/`, `UI/MainMenu/`, `UI/Settings/`, `Gameplay/` (Report 017 — `GameplayBootstrap`, the Gameplay scene's composition root), `CameraControl/` (Report 019 — `CameraController`; named `CameraControl`, not `Camera`, to avoid a namespace/type collision with `UnityEngine.Camera`, see decisions log #51), `Environment/` (Report 030 — `BorderDecorationLayout`, pure geometry for the game board's decorative terrain border) — mirrors the module map in ARCHITECTURE.md §2. | Extends (does not replace) the baseline "grouped by feature/domain, mirrors namespaces" rule. | Derived from GI-1–19 and DESIGN section collectively |
| New `Art/Tiles/` folder (Report 030 — `Tile_Water.asset`) for `UnityEngine.Tilemaps.Tile` assets, alongside the existing `Art/Materials/`/`Art/Shaders/`. | Extends the baseline folder diagram's `Art/` subfolders — Tilemap assets didn't exist in the project before this report. | Derived, Report 030 (first Tilemap content in the project) |
