...
| Buildings | `CaseGame.Buildings` | `BuildingBase` (: `GameEntityBase` — virtual `SpawnPosition` (Report 017, decisions log #42); as of Report 020 also owns its own placement state (`FootprintOrigin`, `SetPlacement(GridModel, origin)`) and overrides `OnEntityDied()` to release that footprint back to the grid — combat death and manual removal both funnel through here, see decisions log #52/#53), `Barracks` (overrides `SpawnPosition`), `PowerPlant`, `BuildingDefinition` (: `GameEntityDefinition`; `ProducibleUnits` is `List<UnitCatalogEntry>`, decisions log #41), `BuildingFactory`; also `BuildingCatalogEntry`/`BuildingCatalog` (Report 014), `BuildingCatalogEntryEventChannel` (decisions log #32), `SelectedBuildingEventChannel` (Report 015) | Entities, Grid, Pooling, Units, Events |
...
| Placement | `CaseGame.Placement` | `BuildingGhostView` (View: toggles a desaturated ghost silhouette, tinted green/red, vs. the real sprite — see decisions log #28) and `PlacementController` (Controller: mouse→cell, validity query, commit-to-grid, and — as of Report 014 — subscribes to `BuildingCatalogEntryEventChannel` to start placement when the Production Menu raises a "produce" request; as of Report 017, `Update()` also skips world input while the pointer is over UI, see decisions log #47). As of Report 020, the cell passed to `UpdateGhostAt`/`TryCommitAt` is the footprint's *center*, not its bottom-left corner — converted internally via `GridModel.FootprintCenterToWorld` — and `TryCommitAt` calls the committed instance's `BuildingBase.SetPlacement` (decisions log #52) | Grid, Buildings, Events |
| Selection | `CaseGame.Selection` | `SelectionController`: left-click select (plain click replaces, shift-click adds/removes a soldier — GI-7/8's "unit(s)"), right-click move-or-attack (GI-7/8/10/11 — attack takes priority over movement when the cursor is over an `IDamageable`). Selecting a building and selecting soldiers are mutually exclusive; visual feedback is `GameEntityBase.SetSelected`, no new prefab wiring. Raises `SelectedBuildingEventChannel` on building-selection change; as of Report 017, `Update()` also skips world input while the pointer is over UI (decisions log #47). As of Report 020, `SetSelectedBuilding` also defensively clears a stale reference to a building that died/was removed elsewhere before comparing — mirrors the existing soldier-pruning pattern (decisions log #39/#53) | Units, Buildings, Grid, Combat, Events |
