...
| CameraControl | `CaseGame.CameraControl` | `CameraController` (Report 019, human-requested quality-of-life, not a brief requirement) — middle-mouse-drag pan, scroll-wheel zoom (clamped), both skipped while the pointer is over UI (same `IsPointerOverGameObject` guard as Placement/Selection, decisions log #47). No `GridModel`/Factory dependency, so unlike the other controllers it needs no `Initialize()` — a direct Inspector-wired `Camera` reference is enough. Named `CameraControl`, not `Camera`, to avoid a namespace/type collision with `UnityEngine.Camera` (decisions log #51) | — |
