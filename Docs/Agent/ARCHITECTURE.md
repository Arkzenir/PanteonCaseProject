...
| Entities | `CaseGame.Entities` | `GameEntityDefinition` (abstract SO: name/sprite/footprint/maxHealth) and `GameEntityBase` (abstract MonoBehaviour: owns a `Health`, implements `IDamageable`, sprite assignment, death callback, `SetSelected(bool)` (Report 015), and — as of Report 020 — a `protected virtual OnEntityDied()` hook called from the death path alongside the pooling callback, so a subclass can add its own death-time cleanup without event-subscription/pooled-reuse hazards; see decisions log #52) — shared by Buildings and Units so neither duplicates this shape (see decisions log #22) | Combat |
