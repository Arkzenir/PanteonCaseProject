...
| Units | `CaseGame.Units` | `SoldierBase` (: `GameEntityBase`, adds `MoveTo`/`TryAttack`; `MoveTo`'s `FollowPath` coroutine walks each path step over a fixed `StepDuration(moveSpeed)` — orthogonal and diagonal steps take the same time, both counting as exactly 1 cell, via the pure/testable `StepDuration`/`InterpolateStep` static helpers, decisions log #57), `Soldier` (the one concrete type, decisions log #26), `UnitDefinition` (: `GameEntityDefinition`; `MoveSpeed` is cells/second, not world-units/second), `UnitFactory` (as of Report 020 also tracks `ActiveUnits` — a live set of everything it's created and hasn't released, used for spawn-cell occupancy checks, decisions log #54); also `UnitCatalogEntry`, `UnitProductionRequest`/`UnitProductionRequestEventChannel`, and `UnitProductionController` (Report 020: now also snaps the spawn position to a grid cell and blocks production if that cell is occupied — by a building or another unit, see decisions log #54) | Entities, Grid, Pathfinding, Combat, Events |
...
| Selection | `CaseGame.Selection` | `SelectionController`: left-click select (plain click replaces, shift-click adds/removes a soldier — GI-7/8's "unit(s)"), right-click move-or-attack (GI-7/8/10/11 — attack takes priority over movement when the cursor is over an `IDamageable`). Selecting a building and selecting soldiers are mutually exclusive; visual feedback is `GameEntityBase.SetSelected`, no new prefab wiring. Raises `SelectedBuildingEventChannel` on building-selection change; as of Report 017, `Update()` also skips world input while the pointer is over UI (decisions log #47). `SetSelectedBuilding` defensively clears a stale reference to a building that died in *combat* before comparing — mirrors the existing soldier-pruning pattern (decisions log #39). Also subscribes to `BuildingRemovalRequestedEventChannel` (`HandleBuildingRemoved`) to proactively clear the selection when the selected building is manually removed — removal doesn't set `IsDead` the way combat death does, so the lazy staleness check alone can't catch it (decisions log #55) | Units, Buildings, Grid, Combat, Events |
